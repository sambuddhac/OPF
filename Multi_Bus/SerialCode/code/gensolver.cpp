//solver.cpp
/* Produced by CVXGEN, 2014-08-13 00:42:14 -0400.  */
...
double Gensolver::eval_objv(void) {
  ...
  objv += params.c0[0]+work.quad_155015135232[0]+work.quad_497818112000[0];
  return objv;
}
...
void Gensolver::refine(double *target, double *var) {
  ...
  for (j = 0; j < settings.refine_steps; j++) {
    ...
    if (settings.verbose_refinement) {
      if (j == 0)
        cout << "Initial residual before refinement has norm squared " << norm2 << "\n";
      else
        cout << "After refinement we get squared norm " << norm2 << "\n";
    }
#endif
...
long Gensolver::solve(void) {
...
#ifndef ZERO_LIBRARY_MODE
  if (settings.verbose)
    cout << "iter     objv        gap       |Ax-b|    |Gx+s-h|    step" << "\n";
#endif
  fillq();
  fillh();
  fillb();
  ...
  for (iter = 0; iter < settings.max_iters; iter++) {
    ...
    if (settings.verbose) {
      ...
      cout << iter+1 << "   " << work.optval << "  " << work.gap << "  " << sqrt(work.eq_resid_squared) << "  " << sqrt(work.ineq_resid_squared) << "  " << alpha << "\n";
    }
#endif
...
void Gensolver::multbyP(double *lhs, double *rhs) {
  ...
  lhs[0] = rhs[0]*(2*(params.c2[0]+work.frac_121674190848));
  lhs[1] = rhs[1]*(2*work.frac_121674190848);
}
void Gensolver::fillq(void) {
  work.q[0] = params.c1[0]+2*(-params.Pg_N_init[0]+params.Pg_N_avg[0]+params.ug_N[0])*work.frac_121674190848;
...
void Gensolver::fill_KKT(void) {
  work.KKT[7] = 2*(params.c2[0]+work.frac_121674190848);
...
//Workspace work;
//Settings settings;
Gensolver::Gensolver( double cost2, double cost1, double cost0, double PgenMax, double PgenMin ) {
  params.c2[0] = cost2;
  params.c1[0] = cost1;
  params.c0[0] = cost0;
  params.rho[0] = 1.0;
...
double Gensolver::getPMax(){return params.PgMax[0];}
double Gensolver::getPMin(){return params.PgMin[0];}
double Gensolver::getQuadCost(){return params.c2[0];}
double Gensolver::getLinCost(){return params.c1[0];}
double Gensolver::getNLCost(){return params.c0[0];}
