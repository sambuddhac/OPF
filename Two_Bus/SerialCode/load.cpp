...
Load::Load( int idOfLoad, Node *nodeConnl, Loadsolver &paramOfLoad, double Load_P ) // constructor begins
	: loadID( idOfLoad ),
	  Pl( Load_P ),
	  connNodelPtr( nodeConnl ),
	  loadSolver( paramOfLoad ),
	  Thetal( 0.0 )
{
	//cout << "\nInitializing the parameters of the load with ID: " << loadID << endl;
	connNodelPtr->setlConn(); // increments the load connection variable to node
...
{
	//**loadSolver.mainsolve( lRho, vprevavg, Aprevavg, vprev );
	//**double Thetal = loadSolver.getThetaSol();
...
{
	//double Thetal = loadSolver.getThetaSol();
	//double Thetal = vprevavg + Aprevavg - vprev;
