...
void Network::generatorThread( int threadCount, int sizeOfBlock, int iterationCount, double uPrice[], double vPrice[], double Rho, double pavBuffer[], double V_avg[], double angleBuffer1[], int indexOfBuffer ) // Distributed Generators' Optimization Problem
{
...
void Network::runSimulation() //Function runSimulation begins
{
	...
	while( ( primalTol >= 0.001 ) || ( dualTol >= 0.002 ) ) { // ( iteration_count <= 122 )
		...
		// loop for spawning threads, calculating the results and storing them in container
			...
			threads[ thCount ] = thread ( &Network::generatorThread, this, thCount, block_size, iteration_count, uPrice, vPrice, Rho, pavBuffer, V_avg, angleBuffer1, bufferIndex ); 
			//**threads[ thCount ] = thread ([thCount, block_size, iteration_count, uPrice, vPrice, Rho, pavBuffer, V_avg, angleBuffer1, bufferIndex, this] {generatorThread(thCount, block_size, iteration_count, uPrice, vPrice, Rho, pavBuffer, V_avg, angleBuffer1, bufferIndex);});
    		} 
		...
		objectiveValue.push_back( 2 ); // record the objective values
		iteration_count++;
